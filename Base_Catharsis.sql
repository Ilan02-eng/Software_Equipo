...
ALTER TABLE Run
    ADD CONSTRAINT pk_run PRIMARY KEY (run_ID);

ALTER TABLE Enemy
    ADD CONSTRAINT pk_enemy PRIMARY KEY (enemy_ID);

ALTER TABLE Cards
    ADD CONSTRAINT pk_cards PRIMARY KEY (card_ID);

ALTER TABLE Combat
    ADD CONSTRAINT pk_combat PRIMARY KEY (combat_ID);

ALTER TABLE Run_Cards
    ADD CONSTRAINT pk_run_cards PRIMARY KEY (card_ID, run_ID, combat_ID);

ALTER TABLE Combat_Stats
    ADD CONSTRAINT pk_combat_stats PRIMARY KEY (combat_stats_ID);
...
ALTER TABLE Run
    ADD CONSTRAINT fk_run_player
    FOREIGN KEY (player_ID) REFERENCES Player(player_ID)
    ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE Combat
    ADD CONSTRAINT fk_combat_run
    FOREIGN KEY (run_ID) REFERENCES Run(run_ID)
    ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE Combat
    ADD CONSTRAINT fk_combat_enemy
    FOREIGN KEY (enemy_ID) REFERENCES Enemy(enemy_ID)
    ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE Run_Cards
    ADD CONSTRAINT fk_runcards_card
    FOREIGN KEY (card_ID) REFERENCES Cards(card_ID)
    ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE Run_Cards
    ADD CONSTRAINT fk_runcards_run
    FOREIGN KEY (run_ID) REFERENCES Run(run_ID)
    ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE Run_Cards
    ADD CONSTRAINT fk_runcards_combat
    FOREIGN KEY (combat_ID) REFERENCES Combat(combat_ID)
    ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE Combat_Stats
    ADD CONSTRAINT fk_combatstats_combat
    FOREIGN KEY (combat_ID) REFERENCES Combat(combat_ID)
    ON DELETE CASCADE ON UPDATE CASCADE;
...
(3, 'Abandoned', '2024-01-13 11:00:00'),
...
('Attack',  'Fireball',    'AoE fire damage',     'All',   'dmg+15',  'Energy', 3, 'sprites/fireball.png'),
('Heal',    'Potion',      'Recover HP',          'Self',  'hp+20',   'Energy', 2, 'sprites/potion.png'),
('Debuff',  'Poison Dart', 'Apply poison DoT',    'Enemy', 'dot+5',   'Energy', 2, 'sprites/dart.png');
